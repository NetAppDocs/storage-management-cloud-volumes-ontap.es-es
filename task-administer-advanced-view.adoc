...
[role="lead"]
Las funciones avanzadas de gestión de almacenamiento en Cloud Volumes ONTAP están disponibles a través del Administrador del Sistema de ONTAP , una interfaz de gestión incluida con los sistemas ONTAP . Puede acceder a System Manager directamente desde la consola de NetApp .
...
* Niveles de nube de NetApp : Cloud Volumes ONTAP no admite niveles de nube.  Debe configurar la clasificación de datos en el almacenamiento de objetos directamente desde la Vista estándar al crear volúmenes.
