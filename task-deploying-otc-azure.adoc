...
---
= Implementa Cloud Volumes ONTAP usando opciones avanzadas en Azure
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
** Deberías tener un https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["Agente de consola asociado con su sistema"^].
...
. Si se te solicita, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["crear un agente de consola"^].
...
| Editar credenciales | Puedes elegir diferentes credenciales de Azure y una suscripción de Azure distinta para utilizarlas con este sistema Cloud Volumes ONTAP. Necesitas asociar una suscripción de Azure Marketplace con la suscripción de Azure seleccionada para poder implementar un sistema Cloud Volumes ONTAP de pago por uso. https://docs.netapp.com/us-en/console-setup-admin/task-adding-azure-accounts.html["Aprenda cómo agregar credenciales"^]. 
|===
. *Servicios*: habilite o deshabilite los servicios individuales que desea o no desea utilizar con Cloud Volumes ONTAP.
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["Obtenga más información sobre la NetApp Data Classification"^]
...
TIP: Si la cuenta de Azure que estás utilizando tiene la https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["permisos requeridos"^], la consola elimina los recursos de Cloud Volumes ONTAP de un grupo de recursos en caso de fallo en la implementación o eliminación.
...
| Utilizar los existentes | Si eliges un grupo de seguridad ya existente, este debe cumplir los requisitos de Cloud Volumes ONTAP. link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#security-group-rules["Ver el grupo de seguridad predeterminado"^]. 
|===
...
NOTE: Si existe una versión Release Candidate, General Availability o patch más reciente para la versión seleccionada, entonces Console actualiza el sistema a esa versión al crear el entorno de trabajo. Por ejemplo, la actualización ocurre si seleccionas Cloud Volumes ONTAP 9.16.1 P3 y está disponible 9.16.1 P4. La actualización no ocurre de una versión a otra, por ejemplo, de 9.15 a 9.16.
...
+
Si necesitas ayuda para elegir el tipo y el tamaño del disco, consulta link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Dimensionar su sistema en Azure"^].
...
+
Esta opción solo está disponible para determinados tipos de máquinas virtuales. Para saber qué tipos de máquinas virtuales son compatibles, consulta link:reference-configs-azure.html#ha-pairs["Configuraciones admitidas por licencia para pares HA"].
...
+
Para más información, consulta link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile["Comprensión de los perfiles de uso del volumen"^] y link:concept-data-tiering.html["Descripción general de la clasificación de datos"^].
...
** El tamaño del disco es para todos los discos en el agregado inicial y para cualquier agregado adicional que la Consola crea cuando utiliza la opción de aprovisionamiento simple.  Puede crear agregados que utilicen un tamaño de disco diferente mediante la opción de asignación avanzada.
+
Si necesitas ayuda para elegir el tamaño del disco, consulta link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Dimensione su sistema en Azure"^].
...
+
link:reference-configs-azure.html#ha-pairs["Obtenga más información sobre las configuraciones compatibles en Azure"^].
