...
summary: 'Configura tu red de Google Cloud para que los sistemas Cloud Volumes ONTAP puedan funcionar correctamente. Aprenderás sobre los requisitos de red, direcciones IP privadas, VPC, subredes, duplicación de paquetes, reglas de firewall y configuraciones de alta disponibilidad (HA) para CVO, además de los requisitos del agente de Console.' 
---
...
https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Revise los permisos de VPC compartidos necesarios que se tratan en la implementación del agente de consola"^]
...
El agente de Console también se comunica con varios puntos finales para las operaciones diarias. Para obtener información sobre los puntos finales, consulta https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["Ver los puntos finales contactados desde el agente de la consola"^] y https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["Preparar la red para usar la consola"^].
...
TIP: ¿Buscas información sobre el agente de Console? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-gcp.html["Ver las reglas de firewall para el agente de la consola"^]
...
=== Configuración de Infrastructure Manager en modo privado o en entornos protegidos

Si deseas implementar Cloud Volumes ONTAP 9.16.1 o una versión posterior en modo privado o en un proyecto protegido por VPC Service Controls, debes realizar algunos cambios en la configuración para que Cloud Volumes ONTAP pueda utilizar Google Cloud Infrastructure Manager como servicio de implementación en lugar de Deployment Manager, que Google dejará de ofrecer en el futuro.

.Antes de empezar
* Asegúrate de que tu sistema Cloud Volumes ONTAP sea 9.16.1 o posterior. Si no es así, actualiza tu sistema. Consulta https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html["Actualizar Cloud Volumes ONTAP"] para ver las instrucciones.
* Asegúrate de que las API de Google Cloud estén habilitadas. Consulta https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis["Habilita las API de Google Cloud"^].
* Asegúrate de que la API Cloud Build está activada. Consulta https://console.cloud.google.com/flows/enableapi?apiid=cloudbuild.googleapis.com&redirect=https://console.cloud.google.com/build/docs/private-pools/set-up-private-pool-environment["Habilita la API Cloud Build aquí"^].
* Verifica que la cuenta de servicio del agente de la Console tenga todos los permisos estándar. Además, asegúrate de que la cuenta de servicio tenga los permisos `cloudbuild.workerpools.get` y `cloudbuild.workerpools.list`. Consulta https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#service-account-permissions["Permisos de Google Cloud para el agente de la consola"^].


.Pasos
. Crea un grupo de trabajadores privado con esta configuración en la misma región que los despliegues de Cloud Volumes ONTAP. Para información sobre cómo crear un grupo de trabajadores privado, consulta  https://docs.cloud.google.com/build/docs/private-pools/create-manage-private-pools#creating_a_new_private_pool["Documentación de Google Cloud: crear y gestionar pools privados"^] y  https://cloud.google.com/build/pricing["Precios de Google Cloud Build"^].
+
Si implementas Cloud Volumes ONTAP en un proyecto protegido por VPC Service Controls, crea el grupo de trabajadores privado en el mismo proyecto que el agente de la Console. El grupo de trabajadores debe estar en estado en ejecución. El grupo de trabajadores público predeterminado de Cloud Build de Google Cloud no puede ejecutarse dentro de un perímetro de servicio.
...
** La subred configurada para acceder a https://cloud.google.com/apis["APIs de Google Cloud"^]. Realiza estos pasos para asegurarte de que la subred puede acceder a Google APIs:
+
...
. Cuando despliegues Cloud Volumes ONTAP en modo privado, selecciona el nombre de este worker pool en el campo *GCP Worker Pool*. Consulta link:task-deploying-gcp.html["Lanzamiento de Cloud Volumes ONTAP en Google Cloud"] para instrucciones.
+
Si implementas Cloud Volumes ONTAP en un proyecto protegido por VPC Service Controls, crea el sistema Cloud Volumes ONTAP mediante la API e incluye el parámetro  `workerPool` en el cuerpo de la solicitud de creación.

+

NOTE: Actualmente, no se admite la inclusión de un conjunto de trabajadores al implementar Cloud Volumes ONTAP mediante la consola UI. Para los proyectos protegidos por VPC Service Controls, la API es la única forma admitida para implementar Cloud Volumes ONTAP.
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Ver los requisitos de red para el agente de consola"^]
...
Para obtener información sobre cómo configurar servidores proxy para el agente de la consola, consulta el  https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["Configurar un agente de consola para utilizar un servidor proxy"^].
