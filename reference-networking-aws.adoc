...
Durante la implementación de Cloud Volumes ONTAP, el campo proxy del mediador de AWS acepta una dirección de servidor. Si introduces un nombre de dominio completo (FQDN) en lugar de una dirección IP, el FQDN no se resolverá a menos que la VPC de AWS esté configurada con un servidor DNS personalizado. Esto es porque el DNS predeterminado proporcionado por AWS solo resuelve registros DNS públicos y hosts internos de AWS, y no puede resolver FQDN externos como servidores proxy de clientes. Si este escenario aplica para ti, tienes dos opciones:

* Agrega la dirección IP del servidor proxy directamente en el campo del proxy del mediador en lugar del dominio completo.
* Configura un conjunto de opciones DHCP en tu VPC que incluya tus direcciones de servidor DNS personalizadas. Esto permite que AWS resuelva el dominio completo (FQDN) cuando lo ingreses en el campo del servidor proxy durante la implementación. Consulta https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["Documentación de AWS"^] para más detalles.
...
TIP: ¿Buscas información sobre el agente de Console? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html["Ver las reglas del grupo de seguridad para el agente de la consola"^]
...
Si quieres usar EBS como capa de rendimiento y Amazon S3 como capa de capacidad, debes asegurarte de que Cloud Volumes ONTAP tiene una conexión a S3. La mejor manera de proporcionar esa conexión es creando un VPC Endpoint para el servicio S3. Para instrucciones, consulta https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["Documentación de AWS: Creación de un punto final de puerta de enlace"^].
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["Ver los requisitos de red para el agente de consola"^]
