...
Discos:: Los datos de los clientes residen en blobs de página de Premium Storage. Cada nodo tiene acceso al almacenamiento del otro nodo. También se necesita almacenamiento adicional para link:reference-configs-azure.html#azure-ha-pair["datos de arranque, raíz y núcleo"].
...
NOTE: A partir de Cloud Volumes ONTAP 9.15.0P1, los blobs de página de Azure ya no son compatibles con las nuevas implementaciones de pares de alta disponibilidad. Si actualmente usas blobs de página de Azure en implementaciones existentes de pares de alta disponibilidad, puedes migrar a tipos de instancia de VM más recientes en las VM de la serie Edsv4 y Edsv5. link:reference-configs-azure.html#ha-pairs["Obtenga más información sobre las configuraciones compatibles en Azure"].
...
Discos:: Los datos de los clientes residen en discos gestionados con almacenamiento localmente redundante (LRS). Cada nodo tiene acceso al almacenamiento del otro nodo. También se requiere almacenamiento adicional para link:reference-configs-azure.html#azure-ha-pair["datos de arranque, raíz, raíz del socio, núcleo y NVRAM"].
...
Discos:: Los datos de los clientes se almacenan en discos gestionados con almacenamiento redundante por zona (ZRS). Cada nodo tiene acceso al almacenamiento del otro nodo. También se requiere almacenamiento adicional para link:reference-configs-azure.html#azure-ha-pair["arranque, raíz, raíz del socio y datos del núcleo"].
Cuentas de almacenamiento:: Las cuentas de almacenamiento se utilizan para implementaciones basadas en discos administrados para manejar registros de diagnóstico y niveles de almacenamiento de blobs.
...
NOTE: Cloud Volumes ONTAP utiliza balanceadores de carga en modo activo-pasivo para las sondas. En Azure, solo el nodo activo responde a una sonda. El nodo pasivo puede aparecer como inactivo con una marca X. Esto es un comportamiento esperado y no es motivo de preocupación.

.Enlace relacionado
link:task-azure-high-availability-mode.html["Habilitar el modo de alta disponibilidad para Cloud Volumes ONTAP en Azure"^]
