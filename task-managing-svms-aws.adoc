...
Ten en cuenta que una LIF de gestión opcional para una VM de almacenamiento requiere una dirección IP privada en un sistema de nodo único y en un par HA en una sola AZ. Esta LIF de gestión proporciona una conexión a herramientas de gestión como SnapCenter.
...
+
** sistema de nodo único: se requiere una dirección IP privada secundaria sin usar.
+
...
== Crea una máquina virtual de almacenamiento en un sistema de nodo único

Estos pasos crean una nueva VM de almacenamiento en un sistema de nodo único. Se necesita una dirección IP privada para crear una LIF de NAS y otra dirección IP privada opcional si quieres crear una LIF de gestión.
...
** `-service-policy default-data-iscsi` indica que una dirección IP no migra entre nodos.
