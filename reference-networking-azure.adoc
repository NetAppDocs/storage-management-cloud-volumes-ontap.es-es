...
Para obtener información sobre cómo configurar servidores proxy, consulta la  https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["Configurar el agente de la consola para utilizar un servidor proxy"^].
...
=== Redes para el cifrado de Azure VNet

Cloud Volumes ONTAP soporta https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-encryption-overview["Cifrado de Azure Virtual Network (VNet)"^] para cifrar el tráfico VM-to-VM dentro de una VNet o entre VNets conectadas por emparejamiento. Esta función se configura en la capa VNet de Azure y es independiente de la topología de Cloud Volumes ONTAP (nodo único o HA).

Sólo tienes que asegurarte de que Accelerated Networking está habilitado en las NIC de la máquina virtual y revisar los requisitos y limitaciones de cifrado de Azure VNet antes de habilitar la función. No deberías modificar los objetos de equilibrador de carga gestionados por NetApp.

https://learn.microsoft.com/en-us/azure/virtual-network/accelerated-networking-overview?tabs=NetworkManager["Documentación de Azure: encriptación VNet y Accelerated Networking"^].
...
La consola crea grupos de seguridad de Azure que incluyen las reglas de tráfico entrante y saliente necesarias para que Cloud Volumes ONTAP funcione correctamente. https://docs.netapp.com/us-en/console-setup-admin/reference-ports-azure.html["Ver las reglas del grupo de seguridad para el agente de la consola"^].
...
No recomendamos modificar los grupos de seguridad predefinidos ni utilizar grupos de seguridad personalizados. Sin embargo, si tienes que hacerlo, ten en cuenta que el proceso de implementación requiere que el sistema Cloud Volumes ONTAP tenga acceso completo dentro de su propia subred. Después de completar la implementación, si decides modificar el grupo de seguridad de red, asegúrate de mantener abiertos los puertos del clúster y los puertos de red de HA, porque la red de HA depende de una comunicación sin restricciones entre los dos nodos. Además, las direcciones IP de los nodos se definen automáticamente durante la implementación. Esto garantiza una comunicación fluida dentro del clúster de Cloud Volumes ONTAP (comunicación de cualquier nodo a cualquier otro).
...
NOTE: Los sistemas de HA tienen menos reglas de entrada que los sistemas de nodo único porque el tráfico de datos de entrada pasa por el Azure Standard Load Balancer. Por esto, el tráfico del Load Balancer debe estar abierto, como se muestra en la regla "AllowAzureLoadBalancerInBound".
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["Ver los requisitos de red para el agente de consola"^]
