...
Para obtener detalles sobre los precios, visite https://bluexp.netapp.com/pricing/["Sitio web de la consola de NetApp"^] .
...
Con licencias basadas en capacidad, la cantidad máxima de sistemas Cloud Volumes ONTAP está limitada a 24 por cuenta de consola de NetApp .  Un _sistema_ es un par de Cloud Volumes ONTAP HA, un sistema de nodo único de Cloud Volumes ONTAP o cualquier máquina virtual de almacenamiento adicional que usted cree.  La VM de almacenamiento predeterminada no cuenta para el límite.  Este límite se aplica a todos los modelos de licencia.
...
En *Administración > Licencias y suscripciones*, puedes ver 50 TiB cargados contra la licencia _Essentials Single Node_.
