...
summary:  
---
= Implementa Cloud Volumes ONTAP desde Azure Marketplace
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Puedes utilizar la implementación directa de Azure Marketplace para implementar Cloud Volumes ONTAP de forma rápida y sencilla. Desde Azure Marketplace, puedes implementar rápidamente Cloud Volumes ONTAP con solo unos clics y explorar sus características y funcionalidades principales en tu entorno.
...
El sistema Cloud Volumes ONTAP implementado mediante la implementación directa de Azure Marketplace presenta estas características. Ten en cuenta que las características de una instancia independiente implementada a través de Azure Marketplace cambian cuando se detecta en la NetApp Console.

* La última versión de Cloud Volumes ONTAP (9.16.1 o posterior).
* Se ofrece una licencia gratuita de Cloud Volumes ONTAP para una capacidad aprovisionada de hasta 500 GiB, sin soporte de NetApp y sin fecha de caducidad. La capacidad superior a 500 GiB requiere un derecho de licencia válido y puede conllevar cargos. Para acceder a la experiencia completa de Cloud Volumes ONTAP y a las capacidades avanzadas de gestión, conecta tu sistema Cloud Volumes ONTAP a NetApp Console. El descubrimiento y la gestión a través de NetApp Console requieren una suscripción válida de Azure Marketplace asociada a tu cuenta de NetApp Console.
...
.Antes de empezar
* Si tienes previsto detectar y gestionar el sistema implementado en NetApp Console, asegúrate de que la cuenta de NetApp Console esté asociada a una suscripción válida de Azure Marketplace. La detección del sistema en NetApp Console requiere una suscripción válida.
...
.Pasos
. Desde el sitio de Azure Marketplace, busca productos de NetApp.
...
Los sistemas Cloud Volumes ONTAP implementados directamente a través de Azure Marketplace no incluyen soporte de NetApp. Si surge algún problema durante la implementación, puedes solucionarlo por tu cuenta.

.Pasos
. En el sitio web de Azure Marketplace, ve a *Diagnóstico de arranque > Registro en serie*.
...
Puedes descubrir los sistemas Cloud Volumes ONTAP que implementaste usando la implementación directa de Azure Marketplace y gestionarlos en la página *Sistemas* de la Console. El agente de la Console descubre los sistemas, los añade, aplica las licencias necesarias y desbloquea todas las capacidades de la Console para estos sistemas. Se mantiene la configuración original de HA en una sola AZ con discos gestionados PSSD v2, y el sistema se registra en la misma suscripción de Azure y el mismo grupo de recursos que la implementación original.

.Acerca de esta tarea
Al detectar los sistemas Cloud Volumes ONTAP implementados mediante la implementación directa de Azure Marketplace, el agente de la Console realiza las siguientes tareas:
...
.Antes de empezar
Asegúrate de que la implementación se haya completado en Azure Marketplace. El agente de la Console solo puede detectar los sistemas cuando la implementación se ha completado y están disponibles para su detección.
...
CAUTION: Durante la detección, es posible que veas mensajes de error, pero puedes ignorarlos hasta que finalice el proceso de detección. No modifiques las configuraciones de Cloud Volumes ONTAP generadas por el sistema en el portal de Azure Marketplace durante la detección, especialmente las etiquetas del sistema. Cualquier cambio que hagas en estas configuraciones puede provocar un comportamiento inesperado del sistema.
...
* https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-subscriptions.html["Agrega una nueva suscripción de marketplace"^]
