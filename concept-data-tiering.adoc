...
Al habilitar la organización por niveles de datos en Google Cloud, Cloud Volumes ONTAP utiliza discos persistentes como nivel de rendimiento para los datos activos y un depósito de Google Cloud Storage como nivel de almacenamiento para los datos inactivos. Puedes seleccionar un nivel de almacenamiento al crear un sistema y cambiarlo en cualquier momento después. Para obtener información sobre cómo cambiar la clase de almacenamiento, consulta link:task-tiering.html["Agrupar datos inactivos en almacenamiento de objetos de bajo costo"].
...
--
Clases de almacenamiento:: Google Cloud utiliza https://cloud.google.com/storage/docs/storage-classes["clases de almacenamiento"^] para gestionar los costes y la disponibilidad de los datos. Te recomendamos que selecciones una clase de almacenamiento en función de la frecuencia de acceso a los datos que preveas, ya que el acceso a los datos conlleva unos costes más elevados. La clase de almacenamiento predeterminada para los datos por niveles es _Standard Storage_. A menos que se modifique manualmente, todos los datos inactivos conservan la clase de almacenamiento predeterminada. Para cambiar la clase de almacenamiento de los datos inactivos, consulta https://docs.cloud.google.com/storage/docs/managing-lifecycles["Documentación de Google Cloud: gestión del ciclo de vida de los objetos"^].
+
--
La clase de almacenamiento para la clasificación de datos es para todo el sistema, no por volumen.
