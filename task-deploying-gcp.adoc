...
+
** Deberías tener un https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Agente de consola asociado con su sistema"^].
** https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["Debes estar preparado para dejar el agente de consola ejecutándose en todo momento"^].
** La cuenta de servicio asociada al agente de la consola https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html["debe tener los permisos requeridos"^].
...
. Si se te solicita, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["crear un agente de consola"^].
...
| Nombre de la cuenta de servicio | Si planeas usar link:concept-data-tiering.html["niveles de datos"] o https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^] con Cloud Volumes ONTAP, entonces necesitas habilitar *Cuenta de servicio* y seleccionar una cuenta de servicio que tenga el rol predefinido de Storage Admin. link:task-creating-gcp-service-account.html["Aprenda a crear una cuenta de servicio"^]. 
...
Si no ves ningún proyecto adicional en la lista desplegable, entonces aún no has asociado la cuenta de servicio con otros proyectos. Ve a Google Cloud Console, abre el servicio IAM y selecciona el proyecto. Agrega la cuenta de servicio con el rol que usas para la Console a ese proyecto. Tendrás que repetir este paso para cada proyecto.


NOTE: Esta es la cuenta de servicio que has configurado para la NetApp Console, link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["como se describe en esta página"^].

Haga clic en *Agregar suscripción* para asociar las credenciales seleccionadas con una suscripción.

Para crear un sistema Cloud Volumes ONTAP de pago por uso, debes seleccionar un proyecto de Google Cloud que esté asociado a una suscripción a Cloud Volumes ONTAP en Google Cloud Marketplace. Consulta https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Cómo asociar una suscripción de Marketplace con las credenciales de Google Cloud"^].
...
. *Ubicación y conectividad*: selecciona la región y la zona de Google Cloud para tu sistema, elige una política de cortafuegos y confirma la conectividad de red con Google Cloud storage para la organización de datos por niveles.
+
La siguiente tabla describe los campos para los que podría necesitar orientación:
...
. *Paquetes preconfigurados*: selecciona uno de los paquetes para desplegar rápidamente un sistema Cloud Volumes ONTAP o haz clic en *Crear mi propia configuración*. Los paquetes preconfigurados varían con la versión de Cloud Volumes ONTAP seleccionada. Por ejemplo, para Cloud Volumes ONTAP 9.18.1 y versiones posteriores, la Console muestra paquetes con C3 VMs, incluidos discos Hyperdisk Balanced. Puedes modificar las configuraciones, como los parámetros de IOPS y rendimiento, según las necesidades de tu carga de trabajo.
+
...
+
Si necesitas ayuda para elegir el tipo y el tamaño del disco, consulta link:task-planning-your-config-gcp.html#size-your-system-in-google-cloud["Dimensiona tu sistema en Google Cloud"^].

. *Caché Flash, Velocidad de Escritura y WORM*:
+
.. Activa *Flash Cache* o elige *Normal* o *High* velocidad de escritura si lo necesitas.
+
Más información sobre link:reference-configs-gcp.html#flash-cache-support["Flash Cache"] y link:concept-write-speed.html["velocidad de escritura"].

+

NOTE: La opción de velocidad de escritura *Alta* permite alcanzar una alta velocidad de escritura y una unidad máxima de transmisión (MTU) de 8,896 bytes. Además, la MTU más alta de 8,896 requiere la selección de VPC-1, VPC-2 y VPC-3 para la implementación. Para más información sobre VPC-1, VPC-2 y VPC-3, consulta  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-console-agent["Reglas para VPC-1, VPC-2 y VPC-3"^].
...
** Si necesitas ayuda para añadir una cuenta de Google Cloud, consulta  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Configuración y adición de cuentas de Google Cloud para la organización de datos en niveles con 9.6"^].
...
| Servidor NTP | Selecciona *Usar dominio de Active Directory* para configurar un servidor NTP utilizando el DNS de Active Directory. Si necesitas configurar un servidor NTP utilizando una dirección diferente, debes usar la API. Para información, consulta la  https://docs.netapp.com/us-en/console-automation/index.html["Documentación de automatización de la NetApp Console"^] para más detalles. Ten en cuenta que solo puedes configurar un servidor NTP al crear un servidor CIFS. No se puede configurar después de crear el servidor CIFS. 
|===
...
. *Detalles del sistema*: Selecciona un proyecto, especifica un nombre de clúster, selecciona una cuenta de servicio si lo deseas, añade etiquetas si lo deseas y luego especifica las credenciales.
+
La siguiente tabla describe los campos para los que podría necesitar orientación:
...
| Proyecto de Google Cloud y suscripción a Marketplace  a| 
Selecciona el proyecto de Google y la suscripción que deseas utilizar para este sistema. Esta es la cuenta de servicio que has configurado para la Consola. Para obtener más información, consulta link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Documentos de configuración de la consola"^]. La Consola utiliza el proyecto para crear el sistema Cloud Volumes ONTAP y la suscripción para determinar la facturación y el uso. Si no ves ningún proyecto adicional en la lista desplegable, significa que aún no has asociado la cuenta de servicio a otros proyectos. Accede a la Consola de Google Cloud, abre el servicio IAM y selecciona el proyecto. Agrega la cuenta de servicio con el rol que utilizas para la Consola a ese proyecto. Tendrás que repetir este paso para cada proyecto.

Haga clic en *Agregar suscripción* para asociar las credenciales seleccionadas con una suscripción.

Para crear un sistema Cloud Volumes ONTAP de pago por uso, debes seleccionar un proyecto de Google Cloud asociado a una suscripción a Cloud Volumes ONTAP desde Google Cloud Marketplace. Consulta el documento de configuración de la consola: https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Cómo asociar una suscripción de Marketplace con las credenciales de Google Cloud"^].
...
| Nombre de la cuenta de servicio | Si tienes previsto utilizar los servicios link:concept-data-tiering.html["NetApp Cloud Tiering"] o https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["Copia de seguridad y recuperación"^], necesitas activar el interruptor *Service Account* y luego seleccionar la Service Account que tenga el rol predefinido de Storage Admin. 
...
| Nombre de usuario y contraseña de ONTAP | Estas son las credenciales para la cuenta de administrador del clúster de Cloud Volumes ONTAP .  Puede usar estas credenciales para conectarse a Cloud Volumes ONTAP a través de ONTAP System Manager o la CLI de ONTAP .  Mantenga el nombre de usuario predeterminado _admin_ o cámbielo por un nombre de usuario personalizado. 
|===
. *Servicios*: Seleccione los servicios que desea utilizar en este sistema.  Para seleccionar Copia de seguridad y recuperación, o para utilizar NetApp Cloud Tiering, debe haber especificado la cuenta de servicio en el paso 3.
+
...
. *Topología de implementación*: Elige varias zonas de disponibilidad (AZ) (recomendado) o una sola AZ para la configuración de alta disponibilidad (HA).
+
link:concept-ha-google-cloud.html["Obtenga más información sobre los modelos de implementación de HA"^] .

. *Configuración de red y conectividad*: selecciona las regiones y zonas pertinentes. Selecciona cuatro VPC diferentes para la configuración de alta disponibilidad (HA), una subred en cada VPC y luego elige una política de cortafuegos.
+

NOTE: Cloud Volumes ONTAP 9.19.1 y versiones posteriores admiten link:concept-ha-google-cloud.html["Discos compartidos para configuraciones de HA de una sola AZ"], lo que hace que la instancia del mediador sea opcional. En esta configuración, no necesitas ingresar una zona del mediador, la VPC correspondiente ni la subred para la replicación de datos.
...
+
Si eliges uno de los paquetes, solo tienes que especificar un volumen y luego revisar y aprobar la configuración. Los paquetes preconfigurados varían según la versión de Cloud Volumes ONTAP seleccionada y los tipos de configuración. Por ejemplo, si eliges una implementación en una sola AZ para Cloud Volumes ONTAP 9.19.1 o posterior, puedes ver paquetes preconfigurados con máquinas de la serie C3 y discos Hyperdisk Balanced. Para versiones anteriores y múltiples AZ, todavía puedes seleccionar máquinas n2-standard y discos Persistent Disks. Puedes modificar las configuraciones, como los parámetros de IOPS y rendimiento, según las necesidades de tu carga de trabajo.
...
NOTE: La opción de velocidad de escritura *Alta* ofrece una alta velocidad de escritura y una unidad máxima de transmisión (MTU) más alta de 8,896 bytes en los tipos de instancia n2-standard-16, n2-standard-32, n2-standard-48 y n2-standard-64. Además, la MTU más alta de 8,896 requiere seleccionar VPC-1, VPC-2 y VPC-3 para la implementación. La alta velocidad de escritura y una MTU de 8,896 dependen de las funciones y no se pueden desactivar individualmente dentro de una instancia configurada. Para más información sobre VPC-1, VPC-2 y VPC-3, consulta  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-connector["Reglas para VPC-1, VPC-2 y VPC-3"^].
...
| Servidor NTP | Selecciona *Usar dominio de Active Directory* para configurar un servidor NTP utilizando el DNS de Active Directory. Si necesitas configurar un servidor NTP utilizando una dirección diferente, entonces deberías usar la API. Consulta la  https://docs.netapp.com/us-en/console-automation/index.html["Documentación de automatización de la NetApp Console"^] para más detalles. Ten en cuenta que solo puedes configurar un servidor NTP al crear un servidor CIFS. No es configurable después de crear el servidor CIFS. 
|===
. *Perfil de uso, tipo de disco y política de niveles*: elija si desea habilitar las funciones de eficiencia de almacenamiento y cambiar la política de niveles de volumen, si es necesario.
+
Para obtener más información, consultelink:task-planning-your-config-gcp.html#choose-a-volume-usage-profile["Elija un perfil de uso de volumen"^] ,link:concept-data-tiering.html["Descripción general de la clasificación de datos"^] , y https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#["KB: ¿Qué funciones de eficiencia de almacenamiento en línea son compatibles con CVO?"^]
